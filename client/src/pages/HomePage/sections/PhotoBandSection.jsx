...
function PhotoCell({ cell }) {
  return (
    <div
      className={`relative overflow-hidden rounded-xl2 group cursor-pointer md:col-span-${cell.colSpan} md:row-span-${cell.rowSpan} h-full`}
    >
      <img
        src={cell.src}
        alt={cell.alt}
...
function MobileSlider() {
  ...
  return (
    <div>
      {/* Slide unique */}
      <div
        style={{ position: 'relative', borderRadius: '14px', overflow: 'hidden', height: '220px', background: '#F2E5D0' }}
      >
        <img
          key={current}
          src={cell.src}
        ...
        {/* Overlay caption */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(44,30,16,0.60) 0%, transparent 50%)' }} />
        <div style={{ position: 'absolute', bottom: '14px', left: '16px' }}>
          <span style={{ color: 'white', fontSize: '13px', fontWeight: 600, textShadow: '0 1px 4px rgba(0,0,0,0.4)' }}>
            {cell.caption}
          </span>
        </div>
        ...
        <button
          ...
          style={{
            ...
            background: 'rgba(253,250,246,0.90)', border: 'none', cursor: 'pointer',
  ...
  return (
    ...
    >
      <div className="section-inner">
        ...
        <div
          className="reveal"
          style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '40px', alignItems: 'end', marginBottom: '40px' }}
        >
          <div>
            <span
              style={{ display: 'inline-block', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '16px', padding: '6px 12px', borderRadius: '100px', background: 'rgba(201,92,53,0.09)', color: '#C95C35', border: '1px solid rgba(201,92,53,0.18)' }}
            >
              En action
            </span>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', fontWeight: 700, color: '#2C1E10', lineHeight: 1.15 }}>
  ...
  >
    {PHOTO_CELLS.map((cell, i) => (
      <PhotoCell key={i} cell={cell} />
        ...
        {isMobile && (
          <p style={{ color: '#5A4428', fontSize: '15px', lineHeight: 1.75, marginTop: '20px' }}>
            Nos techniciens interviennent dans vos cabinets, connaissent vos logiciels, votre matériel, et parfois même l'historique de vos pannes depuis des années. C'est ça, la vraie proximité.
          </p>
        )}
      </div>
